...
import { LayoutDashboard, MapPin, CalendarCheck, Leaf, Bell, Settings, LogOut, Car, BatteryCharging } from 'lucide-react';
...
export default function DriverSidebar({ mobileOpen, setMobileOpen }) {
  ...
  const navItems = [
    ...
    { label: 'Station Details', path: '/driver/station/stn-01', icon: Car },
    { label: 'Book a Slot', path: '/driver/station/stn-01/book', icon: CalendarCheck },
    { label: 'Route Navigation', path: '/driver/navigation/bkg-8821', icon: MapPin },
  ];
          ...
          <div className="bg-[#211f24] border border-[#494551]/60 rounded-xl p-3.5 space-y-2">
            ...
            <div className="flex items-center justify-between text-xs text-[#948e9c]">
              <span className="flex items-center gap-1">
                <BatteryCharging className="w-3.5 h-3.5 text-[#22C55E]" /> 78% SoC
              </span>
              <span>320 km Range</span>
            </div>
            <div className="w-full h-1.5 bg-[#1d1b20] rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#2D8CFF] to-[#22C55E] rounded-full" style={{ width: '78%' }} />
            </div>
          </div>
